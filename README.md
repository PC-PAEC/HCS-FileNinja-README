...
| Search Function (FILE)# | Runs a case-sensitive search for files that display any of the terms specified in the argument. Separate terms with a `*`. Use [regular expressions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_expressions) for more advanced searches.<br>**NOTE:** If using AI to create your regular expressions, specify "It must be Python-compatible and only use inline flags". To ensure your expressions are correct, always double-check using a [python regular expression visualizer](https://extendsclass.com/regex-tester.html#python).<br>**Example 1:** With the argument set to `park * (?i)test`, any files that either include the word `park` (case-sensitive) or the word `test` (case-insensitive) will be displayed. |
